...
fn update_cursor_position(
    ...
    mut uniforms: ResMut<GameOfLifeUniforms>,
) -> Result {
    let window = windows.single()?;

    if let Some(cursor_position) = window.cursor_position() {
        uniforms.cursor_position = Vec2::new(
            (cursor_position.x / window.width()).clamp(0.0, 1.0),
            1.0 - (cursor_position.y / window.height()).clamp(0.0, 1.0),
        );
    }
    Ok(())
}

fn update_shader_time(time: Res<Time>, mut uniforms: ResMut<GameOfLifeUniforms>) {
